...
// ============================================================
// DECLARAÇÕES FFI — FUNÇÕES RUST (idênticas ao Sprites.c)
// ============================================================
...
extern void            tupi_atlas_registrar(TupiAtlasOpaco*, const char*, unsigned int,
...
// ============================================================
// SHADERS HLSL DO SISTEMA DE SPRITES
// ============================================================

// Vertex: posição 2D + UV → aplica projeção ortográfica
...
// Pixel: amostra a textura e aplica alpha
...
// ============================================================
// ESTADO INTERNO D3D11
// ============================================================

// Referências ao device/context vindas do RendererDX11
// (não temos acesso direto — usamos os ponteiros globais via extern)
...
static ID3D11Buffer*              _cb_proj  = NULL; // b0 — MVP (recebe de Camera)
static ID3D11Buffer*              _cb_alpha = NULL; // b1 — alpha
static ID3D11SamplerState*        _sampler  = NULL;
static ID3D11BlendState*          _blend    = NULL;

// Projeção atual (copiada de tupi_sprite_set_projecao)
static float _projecao[16] = {0};

// Batch Rust global
static TupiBatchOpaco* _batch = NULL;

// Cada vértice de sprite: x, y, u, v  (4 floats = 16 bytes)
#define SPRITE_STRIDE (sizeof(float) * 4)
// VBO para 512 quads × 4 vértices
...
void tupi_sprite_iniciar(void) {
    ...
    // ── Constant Buffers ──────────────────────────────────────
    _cb_proj  = _criar_cb(64); // float4x4 = 64 bytes
    _cb_alpha = _criar_cb(16); // float + float3 pad = 16 bytes

    // ── Sampler: filtro NEAREST, clamp ────────────────────────
    D3D11_SAMPLER_DESC sd = {
        .Filter   = D3D11_FILTER_MIN_MAG_MIP_POINT, // NEAREST
...
void tupi_sprite_set_projecao(const float* mat4) {
    ...
    // Atualiza o CB b0 imediatamente
    if (_cb_proj && _ctx) {
...
// ============================================================
// SISTEMA 1 — CARREGAR / DESTRUIR SPRITE
// ============================================================
...
TupiSprite* tupi_sprite_carregar(const char* caminho) {
    ...
    // Cria textura D3D11 a partir dos pixels validados pelo Rust
    ...
    D3D11_SUBRESOURCE_DATA sd = {
        .pSysMem     = img->pixels,
        .SysMemPitch = (UINT)(img->largura * 4), // 4 bytes por pixel RGBA
    };
    ...
    tex2d->lpVtbl->Release(tex2d); // SRV tem a referência — liberamos o tex2d

    int larg = img->largura;
    int alt  = img->altura;
    tupi_imagem_destruir(img); // Rust libera pixels — GPU já tem a cópia

    TupiSprite* sprite = (TupiSprite*)malloc(sizeof(TupiSprite));
    // Reusa o campo 'textura' para guardar o ponteiro da SRV como uint
    // (o campo é GLuint no GL; aqui guardamos o ponteiro via cast)
    sprite->textura = 0;       // não usado diretamente no DX11
    sprite->_srv    = srv;     // campo extra no Sprites.h DX11
    sprite->largura = larg;
    sprite->altura  = alt;
...
// ============================================================
// SISTEMA 2 — ATLAS (idêntico ao Sprites.c — Rust puro)
// ============================================================
...
int tupi_atlas_obter_uv(TupiAtlas* atlas, const char* nome,
    unsigned int frame, float* u0, float* v0, float* u1, float* v1) {
...
// ============================================================
// SISTEMA 3 — BATCH (acumulação idêntica; flush usa D3D11)
// ============================================================

// Helper: envia um quad para o VBO e faz Draw
static void _dx11_draw_quad(const float* quad,
                              ID3D11ShaderResourceView* srv,
                              float alpha) {
    // Atualiza alpha no CB b1
    if (_cb_alpha) {
    ...
    // Atualiza VBO com os 4 vértices do quad (16 floats)
    D3D11_MAPPED_SUBRESOURCE ms;
    ...
    UINT stride = (UINT)SPRITE_STRIDE;
    UINT offset = 0;
...
void tupi_objeto_enviar_batch(TupiObjeto* obj, int z_index) {
    ...
    // textura_id: usamos o endereço da SRV como ID (único por sprite)
...
void tupi_batch_desenhar(void) {
    ...
    // Bind shaders, layout, CBs e sampler
    ...
    ID3D11ShaderResourceView* srv_atual = NULL;

    for (int i = 0; i < contagem; i++) {
        const TupiItemBatch* item = &itens[i];
        // Recupera SRV a partir do ID (que é o ponteiro da SRV)
        ID3D11ShaderResourceView* srv =
...
// ============================================================
// COMPATIBILIDADE — tupi_objeto_desenhar (modo imediato)
// ============================================================
...
void tupi_objeto_desenhar(TupiObjeto* obj) {
    ...
    ID3D11ShaderResourceView* srv = (ID3D11ShaderResourceView*)spr->_srv;
    _dx11_draw_quad(quad, srv, obj->transparencia);
}
